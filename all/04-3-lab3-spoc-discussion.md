...
> 设计方案:维护一个双向循环链表来记录当前所有物理页帧的页表项,并添加访问位.发生页缺失时,遍历链表,遇到访问位为1的页表项,则把访问位置0;直到遇到的第一个访问位为0的页表项,该页表项被替换.
> 性能比较:访问或修改内存中的页时，clock算法只需修改该页的访问位或修改位，效率比较高,但是LRU算法需要将该页从栈中取出,之后再压入栈，效率就比较低;寻找替换页时,clocl算法需要遍历双向循环链表,LRU算法只需要访问栈底.
