...
class fs:
	...
	if self.inodes[inum].getRefCnt() == 1:
            ...
            self.inodeFree(self.inodes[inum].getAddr())
	...
	p = self.getParent(tfile)
	paInum = self.nameToInum[p]
        paAddr = self.inodes[paInum].getAddr()

        self.data[paAddr].delDirEntry(tfile)
        self.inodes[paInum].decRefCnt()
	...
	paInum = self.nameToInum[parent]
        paAddr = self.inodes[paInum].getAddr()

        # is there room in the parent directory?
	if self.data[paAddr].getFreeEntries() == 0:
            return -1

        # if the newfile was already in parent dir?
	if self.data[paAddr].dirEntryExists(newfile):
            return -1

        # now, find inumber of target
	taInum = self.nameToInum[target]

        # inc parent ref count
	self.inodes[paInum].incRefCnt()
	self.inodes[taInum].incRefCnt()

        # now add to directory
	self.data[paAddr].addDirEntry(newfile, taInum)
	...
	if ftype == 'd':
            newAddr = self.dataAlloc()
            self.inodes[newInum].setAll('d', newAddr, 1)
            self.data[newAddr].setType('d')
            self.data[newAddr].addDirEntry('.', newInum)
            self.data[newAddr].addDirEntry('..', paInum)
            self.inodes[newInum].incRefCnt()
        else:
            self.inodes[newInum].setType(ftype)

        self.inodes[paInum].incRefCnt()
        self.data[paAddr].addDirEntry(newfile, newInum)
